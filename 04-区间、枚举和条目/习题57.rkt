...
(check-expect
 ...
 (place-image (text "-2" 20 "red")
              10 (* 3/4 WIDTH)
              (place-image ROCKET 10 (- HEIGHT ROCKET-GROUNDED-Y) BACKG)))

(check-expect
 (show 53)
 (place-image ROCKET 10 (- HEIGHT 53 ROCKET-GROUNDED-Y) BACKG))

(check-expect (launch "resting" " ") -3)
(check-expect (launch "resting" "a") "resting")
(check-expect (launch -2 "a") -2)
(check-expect (launch 3 " ") (- HEIGHT 3))
...
(check-expect (end? (* HEIGHT 2)) #true)
;; 定义区
...
(define ROCKET-GROUNDED-Y ROCKET-CENTER)
  ...
  (cond
    ...
    [(<= -3 x -1)
     (place-image (text (number->string x) 20 "red")
              ...
              (draw-rocket ROCKET-GROUNDED-Y))]
    [(>= x 0)
     (draw-rocket (+ x ROCKET-GROUNDED-Y))]))
